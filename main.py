...
GITHUB_RAW_URL = "https://github.com/Seekeech/userbot-seekeech-version/blob/main/main.py"  # Укажите URL вашего скрипта
SCRIPT_VERSION = "1.4.88"
...
async def loading_message(event):
    """Команда для имитации загрузки сообщения с анимацией прогресса в процентах и итоговым сообщением."""
    try:
        if not event.out:
            return
        ...
        await msg.edit("Загрузка сообщения...\n0%", parse_mode='html') # Меняем начальное сообщение
        await asyncio.sleep(0.1) # Задержка
        
        if msg: # Проверка существует ли сообщение
            for percent in range(1, total_steps + 1):
                await msg.edit(f"Загрузка сообщения...\n{percent}%", parse_mode='html')
                await asyncio.sleep(0.02)
          
            # Добавляем "Сообщение загружено" после достижения 100%
            await msg.edit(f"Загрузка сообщения...\n100%\nСообщение загружено:\n{message}", parse_mode='html')

    except Exception as e:
        print(f"Ошибка при загрузке сообщения: {e}")
        await event.reply("Произошла ошибка при загрузке сообщения.", parse_mode='html')
...
async def main():
    ...
    print("- Используйте /d (текст) для анимации загрузки сообщения.")
